...
export default {
    ...
    computed: {
        ...mapState(["expected/expenses"]),
        serverItemsLength() {
            return this["expected/expenses"].data?.total;
        },
        pageCount() {
            return this["expected/expenses"].data?.last_page;
        },
        items() {
            return this["expected/expenses"].data?.data;
        }
    },
    props: {
        id: null
    ...
    methods: {
        fetch() {
            ...
            this.$store
                .dispatch("expected/expenses/get", data)
                .then(response => {
                    self.loading = false;
                })
                .catch(error => {
        ...
        add() {
            this.$router.push({ name: "CreateExpectedExpenses" });
        },
        download() {
            let self = this;
            self.loading = true;
            ...
            this.$store
                .dispatch("expected/expenses/export", data)
                .then(response => {
                    self.loading = false;
                })
                .catch(error => {
            ...
            switch (type) {
                ...
                case "Delete":
                    if (
                        ...
                        self.loading = true;
                        this.$store
                            .dispatch("expected/expenses/delete", { id: id })
                            .then(response => {
                                self.loading = false;
                                self.fetch();
                            })
