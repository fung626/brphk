...
class VenueController extends Controller
{
    ...
    public function get(Request $request)
    {
        ...
        if ($request->filled(['per_page', 'page'])) {
            $result = $query->paginate(request('per_page'), ['*'], 'page', request('page'));
            $resource = new VenueCollection($result);
            $response['data'] = [
                'data' => $resource,
                'total' => $result->total(),
    ...
    public function details(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id' => 'required|string|exists:venue,id',
        ]);
        ...
        $result = Venue::with(['user'])->where(['id' => request('id')])->first();
        $response = config('response.common.success');
    ...
    public function update(Request $request)
    {
        ...
        $response = config('response.common.success');
        $response['data'] = Venue::with(['user'])->where(['id' => request('id')])->first();
        return response()->json($response, 200);
    }
    ...
    public function export(Request $request)
    {
        ...
        $path = MyPhpOffice::exportTableWithPath('venue', $rows, $headers, 'pdf');
